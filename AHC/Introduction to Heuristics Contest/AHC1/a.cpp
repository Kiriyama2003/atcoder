...
struct Advertisement {
    ...
    friend ostream &operator<<(ostream &os, const Advertisement &ad) {
...
int calc_score(const Input &input, const vector<Advertisement> &ads) {
    double score = 0.0;
    rep(i,input.count){
        score += calc_score_each(input.requests[i], ads[i]);
    }
    return (ll)1e9 * score / input.count;
}
